...
import func_general as fng
import Radial_Distribution as rda
# ------------------------------------------------------------------------------------
# ------ body
# ------------------------------------------------------------------------------------
...
def radda (radial,angle,dihedral):
    ...
    working_dir = fns.working_path(sys.argv)

    #Chage to Working Directory
    fns.cd_path(working_dir)

    #***********************************************#
    #Start: Read Names of Files                     #
    #***********************************************#
...
if __name__ == '__main__':
    ...
    print(f"\nChoose analysis (yes/no): ")
    radial   = input(f"\nRadial   (yes/no): ")
    angle    = input(f"\nAngle    (yes/no): ")
    dihedral = input(f"\nDihedral (yes/no): ")

    if radial == "yes" or angle == "yes" or dihedral == "yes":
        radda(radial,angle,dihedral)
