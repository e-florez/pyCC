...
def atom_transfer(transfer_list, coordinates_XYZ, distances_dict):
    import pandas as pd
    import numpy as np
    ...
    *** September 2020 by particula94h@gmail.com ***
    ...
    for xyz in coordinates_XYZ:
        ...
        distance_matrix = distances_dict[xyz]
        ...
        # - coordinates matrix (no atoms or labels)
        data_xyz = df.loc[:, df.columns != 'element']
        
        # - list of elements
        # header_distance_matrix = df.loc[:, df.columns == 'element']
        header_distance_matrix = df['element'].tolist()
